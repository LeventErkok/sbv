...
-- >>> genE
-- Satisfiable. Model:
--  e1 = Let "b" (Mul (Val (-1)) (Mul (Val (-3)) (Let "n" (Val 7) (Add (Val 23) (Mul (Val 2) (Mul (Val 3) (Val (-4)))))))) (Mul (Val (-1)) (Var "b")) :: Expr
--  e2 =                                                                                                                                     Val (-2) :: Expr
genE :: IO SatResult
...
--
-- >>> queryE
-- e1: (let b = (-1 * (-3 * (let n = 7 in (23 + (2 * (3 * -4)))))) in (-1 * b))
-- e2: -2
queryE :: IO ()
queryE = runSMT $ do
           e1 :: SExpr <- free "e1"
