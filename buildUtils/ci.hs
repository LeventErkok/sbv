 ...
 where Tweaks{ ghcVersion           = lin1GHCVer
       ...
       body = [ "language: c"
           ...
           ++ [ ""
              ...
              , "script:"
              , "  - if [ \"TRAVIS_OS_NAME\" = \"osx\" ];   then if [ \"x$TEST\" = \"x--enable-tests\" ]; then ${CABAL} new-test -f skipHLintTester -w ${HC} ${TEST} ${BENCH} all; fi; fi"
              , "  - if [ \"TRAVIS_OS_NAME\" = \"linux\" ]; then if [ \"x$TEST\" = \"x--enable-tests\" ]; then ${CABAL} new-test                    -w ${HC} ${TEST} ${BENCH} all; fi; fi"
              , ""
              , "  # cabal check"
              , "  - ${CABAL} check"
              ]
