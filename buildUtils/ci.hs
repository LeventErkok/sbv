 ...
 where Tweaks{ ghcVersion           = lin1GHCVer
       ...
       body = [ "language: c"
           ...
           ++ [ ""
              ...
              , "  - if [ \"$TRAVIS_OS_NAME\" = \"osx\" ]; then brew update; brew upgrade python; curl https://haskell.futurice.com/haskell-on-macos.py | python - --make-dirs --install-dir=$HOME/.ghc-install --cabal-alias=head install cabal-install-head ${TRAVIS_COMPILER}; fi"
