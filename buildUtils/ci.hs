...
appveyor = header ++ body ++ footer
 where Tweaks{heavyTestPercentage, ghcVersion, z3Name, z3Path} = winTweaks
       ...
       body = [ ""
              ...
              , "- cabal install happy"
              , "- cabal new-update"
              , "- cabal new-install --only-dependencies --enable-tests --enable-benchmarks -f skipHLintTester"
              , "- cabal new-build"
              , "- cabal new-test --enable-tests --enable-benchmarks"
              , "- cabal new-test SBVDocTest"
              , "# - cabal new-test SBVHLint"
              , "- cabal check"
              , "- cabal new-sdist"
              ]
