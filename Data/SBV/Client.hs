...
import qualified "template-haskell" Language.Haskell.TH        as TH hiding (nameBase)
#if MIN_VERSION_template_haskell(2,18,0)
import qualified "template-haskell" Language.Haskell.TH.Syntax as TH hiding (nameBase)
#endif
...
mkSymbolicADT typeName = do
     ...
     nm <- TH.newName $ "_undefiner_" ++ thNameToSBV typeName
     ...
     let undefine n
           ...
           | True                     = wrap 0
           where tbase  = thNameToSBV typeName
                 base   = thNameToSBV n
         ...
         body      = foldl TH.AppE (TH.VarE 'undefined)
                                   (names ++ [TH.SigE (TH.VarE 'undefined)
                                                      (foldl TH.AppT (TH.ConT (TH.mkName ('S' : thNameToSBV typeName)))
...
mkADT adtKind typeName params cstrs = do
                           ...
                           (TH.caseE [| sequenceA $(TH.listE [ [| unlitCV (literal $(TH.varE a)) |] | a <- as ]) |]
                                     [ TH.match [p|Just $(TH.varP c)|]
                                                (TH.normalB [| let k = kindOf $(TH.varE inp)
                                                               in SBV $ SVal k (Left (CV k (CADT (thNameToSBV n, $(TH.varE c)))))
                                                            |])
                                                []
                                     , TH.match [p|Nothing|]
                                                (TH.normalB (foldl app (TH.varE (TH.mkName ('s' : thNameToSBV n))) (map TH.varE as)))
                                                []
                                     ]))
                     []
    ...
    fromCVFunName <- TH.newName ("cv2" ++ thNameToSBV typeName)
    addDoc ("Conversion from SMT values to " ++ thNameToSBV typeName ++ " values.") fromCVFunName
    ...
    let fromCVSig = TH.SigD fromCVFunName
        ...
        fromCVCls (nm, args) = do
            ...
            pure $ TH.Clause [TH.LitP (TH.StringL (thNameToSBV nm)), pat]
    ...
    catchAll <- do s <- TH.newName "s"
                   l <- TH.newName "l"
                   let errStr   = TH.LitE (TH.StringL ("fromCV " ++ thNameToSBV typeName ++ ": Unexpected constructor/arity: "))
    ...
    register <- do let concretize b@TH.ConT{}     = b
                   ...
                   pure $ TH.DoE Nothing $ [TH.NoBindS (TH.AppE (TH.AppE (TH.VarE 'registerKind) (TH.VarE st))
                                           ...
                                           | (_, fts) <- cstrs, (_, t, KApp n _) <- fts, n /= thNameToSBV typeName
                                           ] ++ [end]
    ...
    let mkPair a b = TH.TupE [Just a, Just b]
        kindDef = foldl1 TH.AppE [ TH.ConE 'KADT
                                 ...
                                 , TH.ListE [ mkPair (TH.LitE (TH.StringL (thNameToSBV p)))
                ...
                () | hasArbitrary -> pure []
                   ...
                   | True         -> [d|instance {-# OVERLAPPABLE #-} Arbitrary $(pure typeCon) where
                                          arbitrary = error $ unlines [ ""
                                                                      ...
                                                                      , "***"
                                                                      , "*** Default arbitrary instance for " ++ thNameToSBV typeName ++ " is too limited."
                                                                      , "***"
    ...
    let btname = thNameToSBV typeName
    ...
    let -- NB. field count starts at 1!
        ...
        declAccessor c (mbUN, ft, _) i = do
                ...
                extras <- case mbUN of
                            Nothing -> pure []
                            Just un -> do let sun = TH.mkName $ 's' : thNameToSBV un
    ...
    symEnum <- case adtKind of
                ...
                ADTEnum          ->
                  let universe  = TH.listE [TH.conE                          con   | (con, _) <- cstrs]
                      universeS = TH.listE [TH.litE (TH.stringL (thNameToSBV con)) | (con, _) <- cstrs]
                  in [d| instance SatModel $(TH.conT typeName) where
                         ...
                         instance SL.EnumSymbolic $(TH.conT typeName) where
                           ...
                           enumFromThen = smtFunction ("EnumSymbolic." ++ thNameToSBV typeName ++ ".enumFromThen") $ \n1 n2 ->
...
mkCaseAnalyzer kind typeName params cstrs = case kind of
  ...
  where mk = do let typeCon = saturate (TH.ConT typeName) params
                    ...
                    bnm = thNameToSBV typeName
                    cnm = TH.mkName $ "sCase" ++ bnm

                se   <- TH.newName ('s' : bnm)
                fs   <- mapM (\(nm, _) -> TH.newName ('f' : thNameToSBV nm)) cstrs
                ...
                let def = TH.FunD cnm [TH.Clause (map TH.VarP (se : fs)) (TH.NormalB (iteChain (zipWith (mkCase se) fs cstrs))) []]
                    ...
                    mkCase cexpr func (c, fields) = (TH.VarE (TH.mkName ("is" ++ thNameToSBV c)), foldl TH.AppE (TH.VarE func) args)
                       where getters = [TH.mkName ("get" ++ thNameToSBV c ++ "_" ++ show i) | (i, _) <- zip [(1 :: Int) ..] fields]
...
toSBV typeName constructorName = go
  where hasArrows (TH.AppT TH.ArrowT _)   = True
        ...
        go (TH.VarT v) = pure $ KVar (thNameToSBV v)
        ...
        go t
           ...
           = KApp (thNameToSBV c) <$> mapM go ps

        -- giving up
        go t = bad "Unsupported constructor kind" [ "Datatype   : " ++ thNameToSBV typeName
                                                  , "Constructor: " ++ thNameToSBV constructorName
                                                  , "Kind       : " ++ TH.pprint t
                                                  , ""
                                                  , if hasArrows t
...
mkInductionSchema typeName params cstrs extraArgCnt = do
   let btype = thNameToSBV typeName
         ...
         = do as <- mapM (const (TH.newName "a")) flds
              let -- When can we have the inductive hypothesis?
                  ...
                  isRecursive (_, _, k) = case k of
                                            KApp t ps -> t == btype && ps == map (KVar . thNameToSBV) params
                                            _         -> False
