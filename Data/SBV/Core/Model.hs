...
class SMTDefinable a where
  ...
  smtFunction         nm       v = sbvDefineValue (UIPrefix nm v) Nothing   $ UIFun   (v, \st fk -> lambda st TopLevel fk v)
...
retrieveUICode (UIPrefix nm v) st fk ui               = do nm' <- prefixNameToUnique st nm v
