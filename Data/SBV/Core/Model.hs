...
class Uninterpreted a where
  ...
  sbvUninterpret :: UIKind a -> String -> a
...
data UIKind a = UIFree                                           -- ^ completely uninterpreted
              | UIFun   (State -> Kind -> IO ([String], String)) -- ^ has code for SMTLib, with final type of kind (note this is the result
                                                                 -- , not the arguments), which can be generated by calling the function on the state.
              | UICodeC ([String], a)                            -- ^ has code for code-generation, i.e., C
              deriving Functor

-- Get the code associated with the UI
retrieveUICode :: State -> Kind -> UIKind a -> IO UICodeKind
retrieveUICode _  _  UIFree           = pure UINone
retrieveUICode st fk (UIFun f)        = UISMT <$> f st fk
retrieveUICode _  _  (UICodeC (c, _)) = pure $ UICgC c

-- Plain constants
instance HasKind a => Uninterpreted (SBV a) where
  sbvUninterpret uiKind nm
     | UICodeC (_, v) <- uiKind
     = v
     | True
    ...
    where ka = kindOf (Proxy @a)
          result st = do isSMT <- inSMTMode st
                         case (isSMT, uiKind) of
                           (True, UICodeC (_, v)) -> sbvToSV st v
                           _                      -> do newUninterpreted st nm (SBVType [ka]) =<< retrieveUICode st ka uiKind
                                                        newExpr st ka $ SBVApp (Uninterpreted nm) []

-- Functions of one argument
instance (SymVal b, HasKind a) => Uninterpreted (SBV b -> SBV a) where
  sbvUninterpret uiKind nm = f
    where f arg0
           | UICodeC (_, v) <- uiKind, isConcrete arg0
           = v arg0
           | True
           = SBV $ SVal ka $ Right $ cache result
           where ka = kindOf (Proxy @a)
                 ...
                 result st = do isSMT <- inSMTMode st
                                case (isSMT, uiKind) of
                                  (True, UICodeC (_, v)) -> sbvToSV st (v arg0)
                                  _                      -> do newUninterpreted st nm (SBVType [kb, ka]) =<< retrieveUICode st ka uiKind
                                                               sw0 <- sbvToSV st arg0
...
instance (SymVal c, SymVal b, HasKind a) => Uninterpreted (SBV c -> SBV b -> SBV a) where
  sbvUninterpret uiKind nm = f
    where f arg0 arg1
           | UICodeC (_, v) <- uiKind, isConcrete arg0, isConcrete arg1
           = v arg0 arg1
           | True
           = SBV $ SVal ka $ Right $ cache result
           where ka = kindOf (Proxy @a)
                 ...
                 result st = do isSMT <- inSMTMode st
                                case (isSMT, uiKind) of
                                  (True, UICodeC (_, v)) -> sbvToSV st (v arg0 arg1)
                                  _                      -> do newUninterpreted st nm (SBVType [kc, kb, ka]) =<< retrieveUICode st ka uiKind
                                                               sw0 <- sbvToSV st arg0
                                                               sw1 <- sbvToSV st arg1
...
instance (SymVal d, SymVal c, SymVal b, HasKind a) => Uninterpreted (SBV d -> SBV c -> SBV b -> SBV a) where
  sbvUninterpret uiKind nm = f
    where f arg0 arg1 arg2
           | UICodeC (_, v) <- uiKind, isConcrete arg0, isConcrete arg1, isConcrete arg2
           = v arg0 arg1 arg2
           | True
           = SBV $ SVal ka $ Right $ cache result
           where ka = kindOf (Proxy @a)
                 ...
                 result st = do isSMT <- inSMTMode st
                                case (isSMT, uiKind) of
                                  (True, UICodeC (_, v)) -> sbvToSV st (v arg0 arg1 arg2)
                                  _                      -> do newUninterpreted st nm (SBVType [kd, kc, kb, ka]) =<< retrieveUICode st ka uiKind
                                                               sw0 <- sbvToSV st arg0
                                                               sw1 <- sbvToSV st arg1
                                                               sw2 <- sbvToSV st arg2
...
instance (SymVal e, SymVal d, SymVal c, SymVal b, HasKind a) => Uninterpreted (SBV e -> SBV d -> SBV c -> SBV b -> SBV a) where
  sbvUninterpret uiKind nm = f
    where f arg0 arg1 arg2 arg3
           | UICodeC (_, v) <- uiKind, isConcrete arg0, isConcrete arg1, isConcrete arg2, isConcrete arg3
           ...
           where ka = kindOf (Proxy @a)
                 ...
                 result st = do isSMT <- inSMTMode st
                                case (isSMT, uiKind) of
                                  (True, UICodeC (_, v)) -> sbvToSV st (v arg0 arg1 arg2 arg3)
                                  _                      -> do newUninterpreted st nm (SBVType [ke, kd, kc, kb, ka]) =<< retrieveUICode st ka uiKind
                                                               sw0 <- sbvToSV st arg0
                                                               sw1 <- sbvToSV st arg1
                                                               sw2 <- sbvToSV st arg2
                                                               sw3 <- sbvToSV st arg3
...
instance (SymVal f, SymVal e, SymVal d, SymVal c, SymVal b, HasKind a) => Uninterpreted (SBV f -> SBV e -> SBV d -> SBV c -> SBV b -> SBV a) where
  sbvUninterpret uiKind nm = f
    where f arg0 arg1 arg2 arg3 arg4
           | UICodeC (_, v) <- uiKind, isConcrete arg0, isConcrete arg1, isConcrete arg2, isConcrete arg3, isConcrete arg4
           ...
           where ka = kindOf (Proxy @a)
                 ...
                 result st = do isSMT <- inSMTMode st
                                case (isSMT, uiKind) of
                                  (True, UICodeC (_, v)) -> sbvToSV st (v arg0 arg1 arg2 arg3 arg4)
                                  _                      -> do newUninterpreted st nm (SBVType [kf, ke, kd, kc, kb, ka]) =<< retrieveUICode st ka uiKind
                                                               sw0 <- sbvToSV st arg0
                                                               sw1 <- sbvToSV st arg1
                                                               sw2 <- sbvToSV st arg2
                                                               sw3 <- sbvToSV st arg3
...
instance (SymVal g, SymVal f, SymVal e, SymVal d, SymVal c, SymVal b, HasKind a) => Uninterpreted (SBV g -> SBV f -> SBV e -> SBV d -> SBV c -> SBV b -> SBV a) where
  sbvUninterpret uiKind nm = f
    where f arg0 arg1 arg2 arg3 arg4 arg5
           | UICodeC (_, v) <- uiKind, isConcrete arg0, isConcrete arg1, isConcrete arg2, isConcrete arg3, isConcrete arg4, isConcrete arg5
           ...
           where ka = kindOf (Proxy @a)
                 ...
                 result st = do isSMT <- inSMTMode st
                                case (isSMT, uiKind) of
                                  (True, UICodeC (_, v)) -> sbvToSV st (v arg0 arg1 arg2 arg3 arg4 arg5)
                                  _                      -> do newUninterpreted st nm (SBVType [kg, kf, ke, kd, kc, kb, ka]) =<< retrieveUICode st ka uiKind
                                                               sw0 <- sbvToSV st arg0
                                                               sw1 <- sbvToSV st arg1
                                                               sw2 <- sbvToSV st arg2
                                                               sw3 <- sbvToSV st arg3
...
instance (SymVal h, SymVal g, SymVal f, SymVal e, SymVal d, SymVal c, SymVal b, HasKind a)
  ...
  sbvUninterpret uiKind nm = f
    where f arg0 arg1 arg2 arg3 arg4 arg5 arg6
           | UICodeC (_, v) <- uiKind, isConcrete arg0, isConcrete arg1, isConcrete arg2, isConcrete arg3, isConcrete arg4, isConcrete arg5, isConcrete arg6
           ...
           where ka = kindOf (Proxy @a)
                 ...
                 result st = do isSMT <- inSMTMode st
                                case (isSMT, uiKind) of
                                  (True, UICodeC (_, v)) -> sbvToSV st (v arg0 arg1 arg2 arg3 arg4 arg5 arg6)
                                  _                      -> do newUninterpreted st nm (SBVType [kh, kg, kf, ke, kd, kc, kb, ka]) =<< retrieveUICode st ka uiKind
                                                               sw0 <- sbvToSV st arg0
                                                               sw1 <- sbvToSV st arg1
                                                               sw2 <- sbvToSV st arg2
                                                               sw3 <- sbvToSV st arg3
...
instance (SymVal c, SymVal b, HasKind a) => Uninterpreted ((SBV c, SBV b) -> SBV a) where
  sbvUninterpret uiKind nm = let f = sbvUninterpret (curry <$> uiKind) nm in uncurry f

-- Uncurried functions of three arguments
instance (SymVal d, SymVal c, SymVal b, HasKind a) => Uninterpreted ((SBV d, SBV c, SBV b) -> SBV a) where
  sbvUninterpret uiKind nm = let f = sbvUninterpret (uc3 <$> uiKind) nm in \(arg0, arg1, arg2) -> f arg0 arg1 arg2
...
instance (SymVal e, SymVal d, SymVal c, SymVal b, HasKind a)
  ...
  sbvUninterpret uiKind nm = let f = sbvUninterpret (uc4 <$> uiKind) nm in \(arg0, arg1, arg2, arg3) -> f arg0 arg1 arg2 arg3
...
instance (SymVal f, SymVal e, SymVal d, SymVal c, SymVal b, HasKind a)
  ...
  sbvUninterpret uiKind nm = let f = sbvUninterpret (uc5 <$> uiKind) nm in \(arg0, arg1, arg2, arg3, arg4) -> f arg0 arg1 arg2 arg3 arg4
...
instance (SymVal g, SymVal f, SymVal e, SymVal d, SymVal c, SymVal b, HasKind a)
  ...
  sbvUninterpret uiKind nm = let f = sbvUninterpret (uc6 <$> uiKind) nm in \(arg0, arg1, arg2, arg3, arg4, arg5) -> f arg0 arg1 arg2 arg3 arg4 arg5
    where uc6 fn a b c d e f = fn (a, b, c, d, e, f)

-- Uncurried functions of seven arguments
instance (SymVal h, SymVal g, SymVal f, SymVal e, SymVal d, SymVal c, SymVal b, HasKind a)
            => Uninterpreted ((SBV h, SBV g, SBV f, SBV e, SBV d, SBV c, SBV b) -> SBV a) where
  sbvUninterpret uiKind nm = let f = sbvUninterpret (uc7 <$> uiKind) nm in \(arg0, arg1, arg2, arg3, arg4, arg5, arg6) -> f arg0 arg1 arg2 arg3 arg4 arg5 arg6
...
instance MonadIO m => SolverContext (SymbolicT m) where
   ...
   addAxiom nm f                       = do st <- symbolicEnv
                                            (frees, ax) <- liftIO $ axiom st f
                                            liftIO $ modifyState st rDefns (SMTDef nm frees ax :) (return ())
