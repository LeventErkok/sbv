...
class Inductive a steps where
   ...
   inductGeneric tagTheorem cfg nm qResult steps helpers = liftIO $ runSMTWith cfg $ do

        liftIO $ putStrLn $ "Inductive " ++ (if tagTheorem then "theorem" else "lemma") ++ ": " ++ nm

        let (_ros, modulo) = calculateRootOfTrust nm helpers
            _finish        = finishKD cfg ("Q.E.D." ++ modulo)

        mapM_ (constrain . getProof) helpers

        inductiveSteps cfg nm qResult steps helpers
