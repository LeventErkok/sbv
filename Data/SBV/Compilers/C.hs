...
cgen cfg nm st sbvProg = CgPgmBundle $ filt [ ("Makefile",  (CgMakefile,     [genMake driver nm nmd]))
...
-- | Generate a makefile
genMake :: Bool -> String -> String -> Doc
genMake ifdr fn dn = foldr1 ($$) [l | (True, l) <- lns]
 where lns = [ (True, text "# Makefile for" <+> nm <> text ". Automatically generated by SBV. Do not edit!")
             , (True, text "")
...
genCProg rtc fn proto (Result ins preConsts tbls arrs uints axms asgns _) inVars outVars mbRet
...
mergeToLib libName bundles = CgPgmBundle $ sources ++ libHeader : (if anyDriver then [libDriver] else []) ++ [libMake]
  where files       = concat [fs | CgPgmBundle fs <- bundles]
        ...
        libMake     = ("Makefile", (CgMakefile, [genLibMake anyDriver libName sourceNms]))
...
genLibMake :: Bool -> String -> [String] -> Doc
genLibMake ifdr libName fs = foldr1 ($$) [l | (True, l) <- lns]
 where lns = [ (True, text "# Makefile for" <+> nm <> text ". Automatically generated by SBV. Do not edit!")
             , (True,  text "")
             , (True,  text "# include any user-defined .mk file in the current directory.")
             , (True,  text "-include *.mk")
