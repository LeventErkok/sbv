...
cgen cfg nm st sbvProg = CgPgmBundle $ filt [ ("Makefile",  (CgMakefile,     [genMake driver nm nmd flags]))
...
genMake :: Bool -> String -> String -> Maybe Doc -> Doc
genMake ifdr fn dn mbLDFlags = foldr1 ($$) [l | (True, l) <- lns]
 where ifld = isJust mbLDFlags
       ...
       lns = [ (True, text "# Makefile for" <+> nm <> text ". Automatically generated by SBV. Do not edit!")
             ...
             , (ifld, text "LDFLAGS?=" <> fromJust mbLDFlags)
             , (True, text "")
             , (ifdr, text "all:" <+> nmd)
             , (ifdr, text "")
