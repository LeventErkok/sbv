...
pprCType fn (CType (ins, outs)) = retType <+> text fn <> parens (fsep (punctuate comma (map mkP ins ++ os))) <> semi
  where (retType, os) = case outs of
                          [(_, bs)] -> (pprCWord bs, [])
                          _         -> (text "void", map mkO outs)
        mkP (n, bs) = pprCWord bs <+> text n
        mkO (n, bs) = pprCWord bs <+> text "*" <> text n
