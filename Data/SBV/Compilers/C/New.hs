  ...
  where result = CgPgmBundle bundleKind
        ...
        validateCompositeABI = inputOutputValidation `seq` returnValidation
          where inputOutputValidation = foldr seq ()
                  ...
                  , (_, value) <- values
                  , sv <- case value of
                            CgAtomic atomic -> [atomic]
                            CgArray atomics -> atomics
                  ]

                returnValidation = case mbRet of
                                     Just sv -> validate "return" (kindOf sv)
                                     Nothing -> ()
...
mergeDrivers libName inc ds = pre : concatMap mkDFun ds ++ [callDrivers (map fst ds)]
  where pre =  text "/* Example driver program for" <+> text libName P.<> text ". */"
            ...
            $$ text "#include <stdio.h>"
            $$ inc
        mkDFun (f, [_pre, _include, callbacks, _header, body, _post]) = [callbacks, header, body, post]
           where header =  text ""
