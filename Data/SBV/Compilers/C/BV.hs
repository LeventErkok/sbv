...
isWideBV :: Kind -> Bool
isWideBV (KBounded False 1) = False
isWideBV (KBounded _     w) = w `notElem` [8, 16, 32, 64]
isWideBV _                  = False
...
wideBVTypeDecls ks = text . unlines $
 ...
 where decl k = ["#ifndef " ++ guard k
       ...
       guard k = "SBV_BV_" ++ map upper (tag k) ++ "_DEFINED"
...
wideBVRuntime ks asgns = text . unlines . map markUnused $
 ...
 where specials (sv, SBVApp op args) = case op of
         ...
         KindCast fr to  | isBV fr && isBV to -> [(SpecialConvert (isSigned fr) fr to)]
         IEEEFP (FP_Reinterpret fr to) | isBV fr && isBV to -> [(SpecialConvert False fr to)]
         _               -> []
...
wideBVConst k i
  ...
  | True       = Nothing
 where normalized = i `mod` (2 ^ width k)
...
wideBVExpr op svs resultKind args
  ...
  | True = Just $ case (op, args, svs) of
      ...
      (Shr, [a, n], x:_)                -> argCall x (if isSigned (kindOf x) then "ashr" else "lshr") [a, argCall x "shift_amount" [n]]
      ...
      (KindCast fr to, [a], _)          -> namedCall (convertName (isSigned fr) fr to) [a]
      (IEEEFP (FP_Reinterpret fr to), [a], _) | isBV fr || isBV to
...
coreRuntime k =
  ...
  ++ ["static inline " ++ ty ++ " " ++ p ++ "_abs(" ++ ty ++ " a)"
     ,"{"
     ,if isSigned k then "  return " ++ p ++ "_get(a, " ++ show (w - 1) ++ ") ? " ++ p ++ "_neg(a) : a;" else "  return a;"
     ,"}"
     ,""
     ]
 where ty   = cType k
       ...
       signedCompare
         | isSigned k = ["  const bool sa = " ++ p ++ "_get(a, " ++ show (w - 1) ++ ");"
...
conversionRuntime nm signExtend sourceOffset fr to =
  ...
  ++ ["  return " ++ normalize to "r" ++ ";"
     ,"}"
     ,""]
 where available = max 0 (width fr - sourceOffset)
       copied    = min available (width to)
       offset i  = if sourceOffset == 0 then i else i ++ " + " ++ show sourceOffset
       extension
         | signExtend && width to > copied =
             ["  if (" ++ getBit fr "a" (show (width fr - 1)) ++ ") for (i = " ++ show copied ++ "; i < " ++ show (width to) ++ "; ++i) { " ++ setBit to "r" "i" "true" ++ " }"]
         | True = []
...
joinRuntime a b to =
  ...
  ,"  for (i = 0; i < " ++ show (width b) ++ "; ++i) { " ++ setBit to "r" "i" (getBit b "low" "i") ++ " }"
  ,"  for (i = 0; i < " ++ show (width a) ++ "; ++i) { " ++ setBit to "r" ("i + " ++ show (width b)) (getBit a "high" "i") ++ " }"
  ,"  return " ++ normalize to "r" ++ ";"
  ,"}"
  ,""]
...
getBit k value bit
  ...
  | isBV k     = "((((uint64_t) " ++ value ++ ") >> (" ++ bit ++ ")) & UINT64_C(1)) != 0"
...
setBit k value bit bitValue
  ...
  | isBV k     = value ++ " = (" ++ cType k ++ ") ((uint64_t) " ++ value ++ " | ((uint64_t) (" ++ bitValue ++ ") << (" ++ bit ++ ")));"
...
width :: Kind -> Int
width KBool          = 1
width (KBounded _ w) = w
width k              = error $ "SBV->C: Expected a bit-vector kind, received " ++ show k

limbs :: Kind -> Int
limbs k = (width k + 63) `div` 64

topMask :: Kind -> Integer
topMask k
  | r == 0    = (1 `shiftL` 64) - 1
  | otherwise = (1 `shiftL` r) - 1
 where r = width k `mod` 64
